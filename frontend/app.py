...
# Accept bare host ("https://host") or full path ("https://host/api/v1") —
# Render's fromService wiring supplies the bare URL; local default includes the path.
_backend_base = os.getenv("BACKEND_URL", "http://localhost:8000").rstrip("/")
BACKEND_URL = _backend_base if _backend_base.endswith("/api/v1") else f"{_backend_base}/api/v1"
    ...
    with col2:
        ...
        with tab2:
            with st.form("reg_form"):
                ...
                if reg_submitted:
                    try:
                        res = requests.post(f"{BACKEND_URL}/register", json={"username": new_user, "password": new_pass})
                        if res.status_code == 200:
                            data = res.json()
                            st.session_state.token = data["access_token"]
        ...
        with st.chat_message("assistant", avatar="🤖"):
            ...
            with st.spinner("Searching Vector Database..."):
                try:
                    ...
                    res = requests.post(f"{BACKEND_URL}/rag/query", json=payload, headers=headers)
...
def knowledge_base_interface():
        ...
        if st.button("🚀 Upload & Ingest"):
            with st.spinner("Uploading and Processing..."):
                try:
                    ...
                    headers = {"Authorization": f"Bearer {st.session_state.token}"}
                    res = requests.post(f"{BACKEND_URL}/ingest/upload", files=files, headers=headers)
                    if res.status_code == 200:
    ...
    try:
        headers = {"Authorization": f"Bearer {st.session_state.token}"}
        res = requests.get(f"{BACKEND_URL}/ingest/files", headers=headers)
        if res.status_code == 200:
            files = res.json()
            if files:
                ...
                with col_d1:
                    if st.button("🗑️ Delete File", type="primary"):
                        requests.delete(f"{BACKEND_URL}/ingest/files/{to_delete}", headers=headers)
                ...
                with col_d2:
                    if st.button("🔄 Rebuild Vector Index"):
                        with st.spinner("Rebuilding Index... This may take a while."):
                            requests.post(f"{BACKEND_URL}/ingest/rebuild", headers=headers)
